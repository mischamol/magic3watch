...
3. Choose which fields to show: date, steps, distance, heart rate, and battery. Distance uses the native Moyoung distance field; the currently selected unit is baked into its heading as `DIST KM` or `DIST MI`. Rebuild the face after changing the unit setting. Its digit set contains the additional half-width decimal sprite expected by the firmware. The point uses the same 5×5-on-8×24 geometry as the official template-34 face. At every scale the digit width is kept even and the point sprite exactly half-width, as required by the firmware renderer.
4. Select a digital clock, **Binary dots**, **Binary 1s and 0s**, or an analogue clock. Both binary modes use four rows for the two hour digits and two minute digits. Binary dots shows the 8, 4, 2, and 1 header; Binary 1s and 0s omits that header and prints each bit directly. Analogue mode uses the native Moyoung `HAND_HOUR` and `HAND_MINUTE` fields, so the watch rotates the hands.
5. Select an element and change its X/Y position or **Scale**. You can also drag it directly in the preview. Binary dots can be enlarged to 150%; the header-free Binary 1s and 0s mode can be enlarged to 200%.
...
RGB565 has no true alpha channel. Transparent elements are therefore pre-composed against the chosen background. Variable-length values such as steps can show small repeating background areas on highly detailed photographs; a quiet background works best. The binary clocks share physical storage for identical on/off or 0/1 images while retaining all ten logical digit indices required by the firmware. The bit cells are tightly cropped, allowing transparent Binary dots to reach 150% and Binary 1s and 0s to reach 200% while remaining within the 300 KiB unpacked-image limit.
