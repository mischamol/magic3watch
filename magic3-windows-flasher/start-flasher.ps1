...
if ($null -eq $listener) {
    Write-Host 'Geen vrije lokale poort gevonden tussen 8765 en 8795.' -ForegroundColor Red
    exit 1
}

try {
    Write-Host "Magic3-flasher draait op http://localhost:$port/"
    ...
    while ($true) {
        ...
        try {
            ...
            if ($requestLine -match '^GET\s+([^\s]+)\s+HTTP/') {
                $requestPath = ([Uri]::UnescapeDataString($Matches[1]) -split '\?')[0]
            }
            ...
            if ($stream) {
                $body = [System.Text.Encoding]::UTF8.GetBytes('Niet gevonden')
